    ...
    <div class="col-12">
        <div class="row">
            <div class="btn-group col-12 p-0 m-0" role="group" aria-label="Basic example">
                <button @click="changeToStory" class="btn btn-block  my-1 text-light" :class="view === 1?'btn-primary':'btn-secondary'">История</button>
                <button @click="changeToChat" class="btn btn-block  my-1 text-light" :class="view === 2?'btn-primary':'btn-secondary'">Чат</button>
            </div>
        </div>
        ...
        <div style="overflow-y: auto;max-height: 40vh !important;">
            <div v-for="(action,index) in actions" v-if="view===1">
                ...
                <div class="text-right">{{action.created_at}}</div>
                </p>
